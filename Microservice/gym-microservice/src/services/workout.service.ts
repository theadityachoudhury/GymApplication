...
export class WorkoutService {
  ...
  async bookWorkout({
    ...
    try {
      ...
      if (existingBooking.length > 0) {
        logger.warn("Workout is fully booked", { existingBooking });
        throw new HttpError(409, "Workout is fully booked");
      }
  ...
  async getAllBookingsForUser({ userId }: { userId: Types.ObjectId }): Promise<any[]> {
    try {
      // First, find all scheduled bookings that should be marked as finished
      const now = new Date();
      ...
      // Update status for bookings whose end time has passed
      const bookingsToUpdate = scheduledBookings.filter(booking => {
        if (!booking.timeSlotId) return false;
        
        const bookingDate = new Date(booking.date);
        const [endHours, endMinutes] = (booking.timeSlotId as any).endTime.split(":").map(Number);
        
        // Set the end time on the booking date
        const endDateTime = new Date(bookingDate);
        endDateTime.setHours(endHours, endMinutes, 0, 0);
        
        // Return true if end time has passed
        return endDateTime < now;
      });
      
      // Bulk update the bookings that need to be marked as finished
      if (bookingsToUpdate.length > 0) {
        const bookingIds = bookingsToUpdate.map(booking => booking._id);
        await BookingModel.updateMany(
          { _id: { $in: bookingIds } },
          { $set: { state: BookingStateEnum.WAITING_FOR_FEEDBACK } }
        );
        
        logger.info(`Automatically marked ${bookingsToUpdate.length} workouts as finished for user ${userId}`);
      }
      
      // Now fetch all bookings with the updated statuses
      const bookings = await BookingModel.find({
        $or: [
          { clientId: userId },
          { coachId: userId }
