import { bookWorkoutParams } from "@/controllers/workout.controller";
import { BookingModel, BookingStateEnum, FeedbackModel, TimeSlotModel, User, UserRole, WorkoutModel, WorkoutOption } from "@/models";
...
export class WorkoutService {
  ...
  async getAllWorkoutOptions() {
    return await WorkoutOption.find().populate("coachesId", "firstName lastName email");
  }

  async getCoachesByWorkout(workoutId: string) {
    try {
      const coaches = await WorkoutModel.find({ workoutType: workoutId }).populate("coachId");
      logger.info(`Fetched ${coaches.length} coaches for workout ${workoutId}`);
      return coaches.map((entry: any) => entry.coachId);
    } catch (error) {
  ...
  async searchWorkoutUsingFilters({ workoutId, coachId, timeSlotId, date }: FilterOptions): Promise<CoachWorkoutAvailability[]> {
    ...
    if (workoutId) {
      workoutFilter.workoutType = workoutId;
    }
    if (coachId) {
      workoutFilter.coachId = coachId;
    }
    ...
    if (date) {
      ...
      // If date is a string like "26" (just the day)
      else if (typeof date === 'string' && /^\d+$/.test(date)) {
        // Create a date for the specified day in the current month/year
        const today = new Date();
        searchDate = new Date(today.getFullYear(), today.getMonth(), parseInt(date));
      }
      // If date is a string in ISO format or other recognizable format
      else {
        searchDate = new Date(date);
      }

      // Validate the date - make sure it's valid
    ...
    // Filter out time slots that have already passed if the date is today
    ...
    if (validTimeSlots.length === 0) {
      throw new HttpError(404, "No available time slots for today");
    }
    ...
    if (timeSlotId) {
      ...
      if (!requestedTimeSlot) {
        throw new HttpError(404, "Selected time slot is not available");
      }
    }

    logger.info("Searching workouts with filter", { ...workoutFilter, date: formattedDate.toISOString() });

    // Find workouts matching the filter
    const workouts = await WorkoutModel.find(workoutFilter)
      .populate({
        path: "coachId",
        select: "firstName lastName image _id role coachId",
        populate: {
          path: "coachId",
          model: "CoachDetails",
          select: "title about rating"
        }
      })
      .populate("workoutType");

    logger.info("Populated workouts", { count: workouts.length });
    ...
    // Filter time slots that are not booked for that particular date
    const availableAlternativeTimeSlots = validTimeSlots.filter(slot => {
      return !bookings.some(booking => booking.timeSlotId.toString() === slot._id.toString());
    });
    ...
    for (const workout of workouts as unknown as PopulatedWorkout[]) {
      const coach = workout.coachId as PopulatedCoach;
      const coachId = coach._id;
      const workoutType = workout.workoutType as PopulatedWorkoutType;
  ...
  async bookWorkout({
    ...
    try {
      ...
      const coach = await User.findOne({ _id: coachId, role: UserRole.Coach });
      if (!coach) {
  ...
  async getBookingsForUser({ userId, date }: { userId: Types.ObjectId, date: Date }): Promise<any[]> {
    try {
      ...
      const bookings = await BookingModel.find({
        ...
        .populate("timeSlotId")
        .populate("clientId", "_id") // Populate clientId with just _id field
        .populate("coachId", "_id")  // Populate coachId with just _id field
        .populate({
          path: "workoutId",
