import { BookingModel, BookingStateEnum, FeedbackModel } from "@/models";
...
export class FeedbackService {
  ...
  async addFeedback({
    ...
    logger.setContext("FeedbackService");
    try {
      // Add debug logging
  ...
  async getFeedbackForUserCoach(
    ...
    try {
      ...
      let formattedFeedback = feedback.map((fb: any) => ({
        feedbackId: fb._id,
        from: {
          id: fb.from._id,
    ...
    try {
      ...
      feedback.forEach((fb) => {
        if (fb.history && fb.history.length > 0) {
          const latestRating = fb.history[0].rating;
